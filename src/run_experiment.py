...
def run_experiment(dataset_loader, client_id, host, port, num_classes, batch_size, train_epochs, model, loss, metric, optimizer, loss_params, metric_params, optimizer_params, numberOfClients, numberOfRounds,experiment, input_shape, class_names):
    ...
    if(client_id != -1):
        ...
        print(' '.join([
            ...
            '--model', str(model),
            '--dataset_loader', str(dataset_loader),
            '--loss', str(loss),
            '--metric', str(metric),
            '--optimizer', str(optimizer),
            '--loss_params', str(loss_params),
            '--metric_params', str(metric_params),
        ...
        if not os.path.isfile(server_path):
            sys.exit(f"Error: '{experiment}' does not contain valid Client.py or Server.py files.")
        server_process = subprocess.Popen(['python3', server_path, '--numberOfClients', str(numberOfClients), '--numberOfRounds', str(numberOfRounds), '--experiment', str(experiment),'--model', str(model),  '--input_shape', str(input_shape), '--class_names', class_names, '--optimizer_params', str(optimizer_params), '--optimizer', str(optimizer), '--loss', str(loss), '--metric', str(metric), '--batch_size', str(batch_size), '--train_epochs', str(train_epochs), '--datasetLoader', str(dataset_loader)])
        print(' '.join([
            'python3', server_path,
            '--numberOfClients', str(numberOfClients),
            '--numberOfRounds', str(numberOfRounds),
            ...
            '--batch_size', str(batch_size),
            '--train_epochs', str(train_epochs),
            '--datasetLoader', str(dataset_loader)
        ]))
        server_process.wait()
