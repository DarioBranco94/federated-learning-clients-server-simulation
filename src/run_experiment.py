...
def run_experiment(experiment_name, client_id, host, port, num_classes, batch_size, train_epochs, model, loss, metric, optimizer, loss_params, metric_params, optimizer_params, numberOfClients, numberOfRounds):
    ...
    if(client_id != -1):
        ...
        client_process = subprocess.Popen(['python3', client_path, '--id', str(client_id), '--host', str(host), '--port', str(port), '--num_classes', str(num_classes), '--batch_size', str(batch_size), '--train_epochs', str(train_epochs), '--model', str(model), '--dataset_loader', str(experiment_name), '--loss', str(loss), '--metric', str(metric), '--optimizer', str(optimizer), '--loss_params', str(loss_params), '--metric_params', str(metric_params), '--optimizer_params', str(optimizer_params)])
        client_process.wait()
    else:
        server_path = os.path.join(base_path, 'Server.py')
        if not os.path.isfile(server_path):
            sys.exit(f"Error: '{experiment_name}' does not contain valid Client.py or Server.py files.")
        server_process = subprocess.Popen(['python3', server_path, '--numberOfClients', str(numberOfClients), '--numberOfRounds', str(numberOfRounds)])
        server_process.wait()
if __name__ == "__main__":
    ...
    run_experiment(args.experiment, args.id, args.host, args.port, args.num_classes, args.batch_size, args.train_epochs, args.model, args.loss, args.metric, args.optimizer, args.loss_params, args.metric_params, args.optimizer_params, args.numberOfClients, args.numberOfRounds)